...
TRANSACTION_COST_BPS_SCENARIOS = [0, 5, 10] # 0, 5, and 10 basis points per side
...
# =============================================================================
def construct_portfolio_returns(test_queries, model_scores_dict, stock_histories, all_dates, current_transaction_cost_bps):
    """
    ...
    for i, current_date in enumerate(sorted_dates):
        ...
        for model, score_map in model_scores_dict.items():
            ...
            # Get PERMNOs and scores
            long_permnos = longs['PERMNO'].tolist()
            short_permnos = shorts['PERMNO'].tolist()
            long_scores = longs['score'].values
            short_scores = shorts['score'].values

            # Normalize scores to create weights
            long_weights = long_scores / np.sum(long_scores)
            short_weights = short_scores / np.sum(short_scores)

            # Process long positions: compute weighted returns
            long_returns = []
            for permno, weight in zip(long_permnos, long_weights):
                raw_ret = calculate_raw_return(permno, current_date, next_date, stock_histories)
                if raw_ret is not None:
                    long_returns.append(weight * raw_ret)

            # Process short positions: compute weighted returns (negated)
            short_returns = []
            for permno, weight in zip(short_permnos, short_weights):
                raw_ret = calculate_raw_return(permno, current_date, next_date, stock_histories)
                if raw_ret is not None:
                    short_returns.append(weight * -raw_ret)
            ...
            # Calculate portfolio return: weighted average of longs and shorts
            port_return = np.sum(long_returns) + np.sum(short_returns)

            # Apply leverage for 150% gross exposure
            port_return *= 1.5

            # Apply transaction costs (5 bps per side on 300% gross exposure)
            # 300% gross exposure means 3x the portfolio value is traded (1.5x long, 1.5x short)
            # Cost = (current_transaction_cost_bps / 10000) * Gross_Exposure_Multiplier
            # For 5 bps per side, and 300% gross exposure, it's 5 bps * 3 = 15 bps of portfolio value per month
            transaction_cost = (current_transaction_cost_bps / 10000) * 3.0
            port_return -= transaction_cost
...
# 9. MAIN EXECUTION
# =============================================================================
def run_core_backtest_pipeline():
    """Runs the computationally expensive core backtest pipeline once and saves outputs."""
    print("--- Running Core Backtest Pipeline (Stage 1) ---")
    core_pipeline_start_time = time.time()
    ...
    print(f"--- Core Backtest Pipeline (Stage 1) completed in {time.time() - core_pipeline_start_time:.2f} seconds ---")

    # Save core pipeline outputs to checkpoints
    core_outputs_checkpoint_path = os.path.join(CHECKPOINT_DIR, "core_pipeline_outputs.pkl")
    with open(core_outputs_checkpoint_path, 'wb') as f:
        import pickle
        pickle.dump({
            'all_test_queries': all_test_queries,
            'model_scores_all': model_scores_all,
            'stock_histories': stock_histories,
            'monthly_dates_sorted': monthly_dates_sorted
        }, f)
    print(f"Core pipeline outputs saved to {core_outputs_checkpoint_path}")

    return all_test_queries, model_scores_all, stock_histories, monthly_dates_sorted

def main(current_transaction_cost_bps):
    """Main execution function."""
    total_start_time = time.time()
    print(f"Starting RankNet Implementation with {current_transaction_cost_bps} BPS transaction costs")
    print(f"Backtest period: {BACKTEST_START} to {BACKTEST_END}")

    core_outputs_checkpoint_path = os.path.join(CHECKPOINT_DIR, "core_pipeline_outputs.pkl")

    all_test_queries = None
    model_scores_all = None
    stock_histories = None
    monthly_dates_sorted = None

    if RESUME_FROM_CHECKPOINT and os.path.exists(core_outputs_checkpoint_path):
        print(f"Loading core pipeline outputs from checkpoint: {core_outputs_checkpoint_path}")
        try:
            with open(core_outputs_checkpoint_path, 'rb') as f:
                import pickle
                loaded_data = pickle.load(f)
                all_test_queries = loaded_data['all_test_queries']
                model_scores_all = loaded_data['model_scores_all']
                stock_histories = loaded_data['stock_histories']
                monthly_dates_sorted = loaded_data['monthly_dates_sorted']
            print("Core pipeline outputs loaded successfully.")
        except Exception as e:
            print(f"Error loading core pipeline outputs: {e}. Re-running core pipeline.")
            all_test_queries, model_scores_all, stock_histories, monthly_dates_sorted = run_core_backtest_pipeline()
    else:
        all_test_queries, model_scores_all, stock_histories, monthly_dates_sorted = run_core_backtest_pipeline()

    # Step 6: Construct portfolios and evaluate performance
    print("Constructing portfolios and calculating returns...")
    full_port_df, full_cumulative = construct_portfolio_returns(all_test_queries, model_scores_all, stock_histories, monthly_dates_sorted, current_transaction_cost_bps)
    ...
    # Save the returns dataframe for analysis
    try:
        returns_csv_path = f'ranknet_score_weighted_returns_{current_transaction_cost_bps}bps.csv'
        print(f"\nSaving final portfolio returns to {returns_csv_path}...")
        full_port_df.to_csv(returns_csv_path)
        print("Save complete.")
    except Exception as e:
...
if __name__ == "__main__":
    # Run the core backtest pipeline once (or load from checkpoint)
    all_test_queries, model_scores_all, stock_histories, monthly_dates_sorted = run_core_backtest_pipeline()

    # Now, loop through transaction cost scenarios and run Stage 2 (portfolio construction and evaluation)
    for bps_cost in TRANSACTION_COST_BPS_SCENARIOS:
        print(f"\n--- Running Portfolio Construction & Evaluation (Stage 2) for {bps_cost} BPS ---")
        main(bps_cost)